package de.thm.mni.ii.classroom.services.memory

import de.thm.mni.ii.classroom.model.User
import org.springframework.beans.factory.annotation.Autowired
import org.springframework.stereotype.Component
import de.thm.mni.ii.classroom.util.ExtensionsKt.getSha1
...
class UserService {
  ...
  def find(username: String, password: String): Option[User] =
    DB.query("SELECT user_id, prename, surname, email, username, alias, global_role FROM user where username = ? and password = ?", (res, _) =>
      parseResult(res), username, password.getSha1().headOption)
